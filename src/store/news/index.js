...
export const updateNews = ({ _id, title, text }) => (dispatch, getState) => new Promise((resolve, reject) => {
    const data = { title, text }
    dispatch(setIsLoadingNewsForm(true))
    request({ url: `/news/${_id}`, method: 'PATCH', data, getState, dispatch })
        .then(data => {
            dispatch(setNewsList(data))
            resolve(data)
        })
...
export const deleteNews = ({ _id }) => (dispatch, getState) => new Promise((resolve, reject) => {
    dispatch(setIsLoadingNewsList(true))
    request({ url: `/news/${_id}`, method: 'DELETE', getState, dispatch })
        .then(data => {
            dispatch(setNewsList(data))
            resolve(data)
        })
